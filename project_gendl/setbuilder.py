...
def make_negative4(filename, dons, accs):
	comp = str.maketrans('ACGTRYMKWSBDHV', 'TGCAYRKMWSVHDB')

	dseen = {}
	for s1, s2, s3, x in dons:
		sig = s1 + s2 + s3
		dseen[sig] = True

	aseen = {}
	for s1, s2, s3, x in accs:
		sig = s1 + s2 + s3
		aseen[sig] = True
	...
	with gzip.open(filename, 'rt') as fp:
		for line in fp.readlines():
			...
			for i in range(20, len(seq) -22):
				if seq[i:i+2] != 'GT': continue
				s = seq[i-20:i+22]
				if s in dseen: continue
				s1 = s[0:20]
				s2 = s[20:22]
				s3 = s[22:]
				negd[(s1, s2, s3, 0)] = True
			for i in range(20, len(seq) -22):
				if seq[i:i+2] != 'AG': continue
				s = seq[i-20:i+22]
				if s in aseen: continue
				s1 = s[0:20]
				s2 = s[20:22]
				s3 = s[22:]
				nega[(s1, s2, s3, 0)] = True
	dseqs = list(negd.keys())
	aseqs = list(nega.keys())
	random.shuffle(dseqs)
	random.shuffle(aseqs)

	return dseqs[0:len(dons)], aseqs[0:len(dons)]
...
for gen in genomes:
	...
	# negative 4 - "decoy sites"
	nd, na = make_negative4(eie, dons, accs)
	write_fasta(f'data42/{gen}.n4don.fa', 'n4don', nd)
	write_fasta(f'data42/{gen}.n4acc.fa', 'n4acc', na)
